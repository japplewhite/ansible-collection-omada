...
| VLANs / networks | **Read: solid, on our fork. Write: not via a clean API call yet — worked around via browser automation.** | `get_networks()` verified 2026-08-27 against a live controller — legacy `setting/lan/networks` endpoint, paginated. `create_network()` targets that same legacy endpoint and is now confirmed *wrong*: live browser capture shows the real Controller UI creates networks through an entirely different, multi-step **official OpenAPI** flow (`openapi/v1/.../networks/param-check → auto-effect-devices → devices/ports → ports-check → check → confirm`), not a single POST to the legacy path. | Live GET confirmed working end to end via `omada_network_info`. The write flow was captured live (URLs/methods/order confirmed via browser network monitoring) but not the request *bodies* — the tooling available exposed only URL/method/status, and two HAR-export attempts failed. Two real VLANs (Surveillance/30, Business LAN/20) were created this session by driving the Controller UI directly via browser automation instead of the API — a working stopgap, not a REST call an Ansible module can make. | Next step for the real API path: another capture attempt with request bodies intact (proper DevTools body capture, or a clean HAR with Preserve Log surviving the confirm step), or find TP-Link's OpenAPI Swagger schema for `/networks/confirm` if reachable. Full detail in `create_network()`'s docstring on the fork. |
