...
| VLANs / networks | **Partially supported, on our private fork only** (not upstream, not on PyPI) | `get_networks()` (read) added and verified 2026-08-27 against a live controller (firmware 6.2.14.12) — legacy `setting/lan/networks` endpoint, paginated. `create_network()` deliberately left `NotImplementedError`: mutating call, payload not yet verified. | Live GET confirmed working end to end via `omada_network_info`; original guessed endpoint/field names (`vlanId`, integer `purpose`) were wrong and corrected against the real response (`vlan`, string `purpose`) — see `github.com/japplewhite/tplink-omada-api-fork`, branch `feature/vlan-network-support` | Read path is solid. Write path (actually creating VLANs) still needs the same live-verification treatment before it can be trusted — see this project's README "Delivery priority" section. |
